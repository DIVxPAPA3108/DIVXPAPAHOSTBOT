# -*- coding: utf-8 -*-
"""
𝐃𝐈𝐕𝐱𝐏𝐀𝐏𝐀 𝐇𝐎𝐒𝐓𝐈𝐍𝐆 𝐁𝐎𝐓 𝐕𝟑.𝟓 — FULL COLOR BUTTONS + ALL FIXES
...
CREDITS: DIVXPAPA CODER + OMEGA UPGRADE
"""
...
@app.route('/')
def home():
    return "I'm DIVXPAPA HOSTING BOT V3.5"
...
DIVXPAPA_DATA_DIR = os.path.join(BASE_DIR, 'DIVXPAPA_data')
DATABASE_PATH = os.path.join(DIVXPAPA_DATA_DIR, 'DIVXPAPA_bot.db')
RUNNING_SCRIPTS_DB = os.path.join(DIVXPAPA_DATA_DIR, 'running_scripts.json')
REFERRAL_DB = os.path.join(DIVXPAPA_DATA_DIR, 'referrals.json')
PENDING_DIR = os.path.join(DIVXPAPA_DATA_DIR, 'pending_approvals')
...
os.makedirs(DIVXPAPA_DATA_DIR, exist_ok=True)
...
# BAN SYSTEM
# ================================
BANNED_USERS_FILE = os.path.join(DIVXPAPA_DATA_DIR, 'banned_users.json')
banned_users = set()
...
# NOTIFICATION SYSTEM
# ================================
NOTIFIED_USERS_FILE = os.path.join(DIVXPAPA_DATA_DIR, 'notified_users.json')
...
class ReferralSystem:
    ...
    def generate_referral_code(self, user_id):
        code = f"DIVXPAPA{user_id}{random.randint(1000, 9999)}"
...
def command_send_welcome(message):
    ...
    if referral_code and referral_code.startswith('DIVXPAPA'):
        try:
    ...
    try:
        ...
        try:
            response = requests.get(raw_url, timeout=30, headers={
                'User-Agent': 'Mozilla/5.0 (compatible; DIVXPAPABot/3.5)'
            })
        except Exception as e:
        ...
        while True:
            try:
                ...
                if '409' in str(e) or 'Conflict' in str(e):
                    ...
                    print("   Stop the other DIVXPAPA process/device before starting this one.")
